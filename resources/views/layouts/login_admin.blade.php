...
<div class="admin-card">
    ...
    <div class="admin-footer">
        <a href="login_1.html" class="back-link">← Back to Resident / Officer Portal</a>
...
<script>
    ...
    function handleAdminLogin() {
        ...
        if (!email || !pwd) {
            showMsg('Please enter your Gmail and password.', 'error');
            return;
        }
        if (!email.includes('@gmail.com')) {
            showMsg('Only Gmail addresses are accepted for admin access.', 'error');
            return;
        }
        ...
        setTimeout(() => {
            ...
            // Sample: accept any @gmail if ends with specific domain (demo only)
            if (email.endsWith('@gmail.com') && pwd.length >= 6) {
                showMsg('✅ Access granted. Redirecting to dashboard…', 'success');
                setTimeout(() => { window.location.href = '/'; }, 1500);
            } else {
                const remaining = MAX_ATTEMPTS - attempts;
                showMsg(`Invalid credentials. ${remaining} attempt${remaining!==1?'s':''} remaining.`, 'error');
                if (remaining <= 2) {
